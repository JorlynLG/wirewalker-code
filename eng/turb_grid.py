...
for i in range(0,58,2):
	...
	turb_array.extend(e_data["e_turb"].tolist())
	profile_array.extend([i]*len(e_data["e_turb"].tolist()))

#neg_pressure_array = [-x for x in pressure_array]
print(max(turb_array))
v_val=1
h_val=1
...
ymax = 0
plt.figure()
plt.scatter(profile_array,pressure_array,c=turb_array, s = 90,marker=(verts,0))
